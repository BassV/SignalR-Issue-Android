...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // SignalR
        final HubConnection connection = new WebSocketHubConnectionP2("http://3aced4d9.ngrok.io/hubs/chat", "Bearer test");

        connection.addListener(new HubConnectionListener() {
            @Override
            public void onConnected() {
                Log.d("TAAAAAG", "Connected");
            }

            @Override
            public void onDisconnected() {
                Log.d("TAAAAAG", "Disconnected");
            }
            ...
            public void onError(Exception exception) {
                Log.d("TAAAAAG", "Error thrown");
            }
        });

        connection.subscribeToEvent("send message", new HubEventListener() {
            @Override
            public void onEventMessage(final HubMessage message) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        String messagee = message.getArguments()[0].getAsString();
...
/*
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                connection.invoke("SendMessage", "abc", "def");
            }
        }, 100);
*/
        // Buttons
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (newMessage.length() != 0){
                    ...
                    connection.invoke("SendMessage", "convId", newMessage, "someId");
